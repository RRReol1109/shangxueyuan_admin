            ...
            <el-dropdown-menu slot="dropdown">
              ...
              <el-dropdown-item>
                <el-upload
                  ...
                  action="http://bs.hk.darkal.cn/undergraduatePatent/upload?token='AuthenticationToken'"
                >
 ...
 height="600"
    ...
    >
      ...
      <el-table-column
        ...
        label="学生姓名"
      ></el-table-column>
      <el-table-column sortable :show-overflow-tooltip="true" prop="name" align="center" label="名称"></el-table-column>
      <el-table-column sortable :show-overflow-tooltip="true" prop="cate" align="center" label="类别"></el-table-column>
      <el-table-column
        sortable
        :show-overflow-tooltip="true"
      ...
      >
        <template slot-scope="scope">
          <span>{{scope.row.firstInventor | reformPaperFilter}}</span>
        </template>
      </el-table-column>
      <el-table-column
        sortable
    ...
    >
      ...
      >
        ...
        <el-row>
          ...
          <el-col :span="12">
            <el-form-item label="类别" prop="cate">
              <el-input size="normal" v-model="form.cate" style="width:99%"></el-input>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
          <el-col :span="12">
            <el-form-item label="授权号:" prop="authorizationNumber">
              <el-input size="normal" v-model="form.authorizationNumber" style="width:99%"></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="12">
        ...
        <el-row>
          <el-col :span="12">
            <el-form-item label="是否第一发明人:" prop="firstInventor">
...
export default {
  ...
  methods: {
    ...
    async list() {
      ...
      let res = await axios.$post("/undergraduatePatent/list", this.query);
      if (res) {
    ...
    async examineData(flag) {
      for (let i = 0; i < this.checkedList.length; i++) {
        ...
        await axios.$post("/undergraduatePatent/update", this.examineForm);
      }
      this.list();
    ...
    async submitForm(formName) {
      ...
      switch (this.operate) {
        case "add":
          await axios.$post("/undergraduatePatent/add", this.form);
          break;
        case "edit":
          await axios.$post("/undergraduatePatent/update", this.form);
          break;
      }
    ...
    async exportData(flag) {
      let data = "";
      if (flag == "temp") {
        data = await axios.$download("/undergraduatePatent/export?id=-1", {});
      } else {
        data = await axios.$download("/undergraduatePatent/export", {
          params: this.query,
        });
      }
      if (data) {
        ...
        link.href = url;
        link.setAttribute("download", "数据库-学生获得专利.xls");
      ...
      this.$confirm("此操作将永久删除该记录, 是否继续?", "提示", {
        ...
        .then(async () => {
          for (let i = 0; i < vm.checkedList.length; i++) {
            await axios.$post("/undergraduatePatent/delete", {
              undergraduatePatentId: vm.checkedList[i].id,
            });
          }
          this.tableData = [];
          await this.list();
    ...
    async del(row) {
      this.$confirm("此操作将永久删除该记录, 是否继续?", "提示", {
        ...
        .then(async () => {
          console.log(row);
          let undergraduatePatentId = row.id;
          await axios.$post("/undergraduatePatent/delete", {
            undergraduatePatentId: undergraduatePatentId,
          });
          this.list();
          this.$message({
            type: "success",
