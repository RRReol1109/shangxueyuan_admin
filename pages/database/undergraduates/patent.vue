        ...
        <el-row>
          ...
          <el-col :span="12">
            <el-form-item label="授权号" prop="authId">
              <el-input style="width:99%" clearable v-model="ruleForm.authId" placeholder="请输入内容"></el-input>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
  ...
  methods: {
      ...
      this.$confirm("此操作将永久删除该记录, 是否继续?", "提示", {
        ...
        .then(async () => {
          for (let i = 0; i < vm.checkedList.length; i++) {
            await axios.$post("/undergraduatePatentAuthorization/delete", {
              undergraduatePatentAuthorizationId: vm.checkedList[i].id,
            });
          }
          this.tableData = [];
          await this.list();
    ...
    async examineData(flag) {
      for (let i = 0; i < this.checkedList.length; i++) {
        ...
        await axios.$post("/undergraduatePatentAuthorization/update", this.examineForm);
      }
      this.list();
