  ...
  <div>
    ...
    <el-drawer size="60%" style="min-height:500px" title :visible.sync="dialogFormVisible">
      ...
      >
        ...
        <!-- <el-form-item label="优势专业类型" prop="persons">
          <el-col :span="12">
            <label>作者姓名-单位</label>
  ...
  methods: {
      ...
      this.$confirm("此操作将永久删除该记录, 是否继续?", "提示", {
        ...
        .then(async () => {
          for (let i = 0; i < vm.checkedList.length; i++) {
            await axios.$post("/undergraduateAdvantageSubject/delete", {
              undergraduateAdvantageSubjectId: vm.checkedList[i].id
            });
          }
          this.tableData = [];
          await this.list();
    ...
    async examineData(flag) {
      for (let i = 0; i < this.checkedList.length; i++) {
        ...
        await axios.$post("/undergraduateAdvantageSubject/update", this.examineForm);
      }
      this.list();
