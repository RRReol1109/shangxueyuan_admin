            ...
            <el-dropdown-menu slot="dropdown">
              ...
              <el-dropdown-item>
                <el-upload
                  ...
                  action="http://bs.hk.darkal.cn/graduatePapers/upload?token='AuthenticationToken'"
                >
    ...
    >
      ...
      >
        ...
        <el-row>
          <el-col :span="12">
            <el-form-item label="论文名称" prop="paperTitle">
              <el-input size="normal" v-model="form.paperTitle" style="width:99%"></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="12">
        ...
        <el-row>
          ...
          <el-col :span="12">
            <el-form-item label="收录情况" prop="collectionSituation">
              <el-input size="normal" v-model="form.collectionSituation" style="width:99%"></el-input>
            </el-form-item>
          </el-col>
        </el-row>
...
export default {
  ...
  methods: {
    ...
    async list() {
      ...
      let res = await axios.$post("/graduatePapers/list", this.query);
      if (res) {
    ...
    async examineData(flag) {
      for (let i = 0; i < this.checkedList.length; i++) {
        ...
        await axios.$post("/graduatePapers/update", this.examineForm);
      }
      this.list();
    ...
    async submitForm(formName) {
      ...
      switch (this.operate) {
        case "add":
          await axios.$post("/graduatePapers/add", this.form);
          break;
        case "edit":
          await axios.$post("/graduatePapers/update", this.form);
          break;
      }
    ...
    async exportData(flag) {
      let data = "";
      if (flag == "temp") {
        data = await axios.$download("/graduatePapers/export?id=-1", {});
      } else {
        data = await axios.$download("/graduatePapers/export", {
          params: this.query,
        });
      }
      if (data) {
      ...
      this.$confirm("此操作将永久删除该记录, 是否继续?", "提示", {
        ...
        .then(async () => {
          for (let i = 0; i < vm.checkedList.length; i++) {
            await axios.$post("/graduatePapers/delete", {
              graduatePapersId: vm.checkedList[i].id,
            });
          }
          this.tableData = [];
          await this.list();
    ...
    async del(row) {
      this.$confirm("此操作将永久删除该记录, 是否继续?", "提示", {
        ...
        .then(async () => {
          console.log(row);
          let graduatePapersId = row.id;
          await axios.$post("/graduatePapers/delete", {
            graduatePapersId: graduatePapersId,
          });
          this.list();
          this.$message({
            type: "success",
