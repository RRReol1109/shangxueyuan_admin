  ...
  methods: {
    ...
    async examineData(flag) {
      for (let i = 0; i < this.checkedList.length; i++) {
        ...
        await axios.$post("/graduateSubjectInformation/update", this.examineForm);
      }
      this.list();
        ...
        .then(async () => {
          for (let i = 0; i < vm.checkedList.length; i++) {
            await axios.$post("/graduateSubjectInformation/delete", {
              graduateSubjectInformationId: vm.checkedList[i].id,
            });
          }
          this.tableData = [];
          await this.list();
    ...
    async del(row) {
      this.$confirm("此操作将永久删除该记录, 是否继续?", "提示", {
        ...
        .then(async () => {
          console.log(row);
          let graduateSubjectInformationId = row.id;
          await axios.$post("/graduateSubjectInformation/delete", {
            graduateSubjectInformationId: graduateSubjectInformationId,
          });
          this.list();
          this.$message({
            type: "success",
